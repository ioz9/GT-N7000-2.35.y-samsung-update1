...
int mdnie_send_sequence(const unsigned short *seq)
{
	int ret = 0, i = 0;
	...
	while (wbuf[i] != END_SEQ) {
		s3c_mdnie_writel(wbuf[i+1], wbuf[i]*4);
		i += 2;
	}
...
void mDNIe_Set_Mode(Lcd_mDNIe_UI mode, u8 mDNIe_Outdoor_OnOff)
{
	...
	if (mDNIe_Outdoor_OnOff) {
		...
		case mDNIe_COLOR_TONE_1:
			mdnie_send_sequence(tune_color_tone_1);
			break;

		case mDNIe_COLOR_TONE_2:
			mdnie_send_sequence(tune_color_tone_2);
			break;

		case mDNIe_COLOR_TONE_3:
			mdnie_send_sequence(tune_color_tone_3);
			break;
