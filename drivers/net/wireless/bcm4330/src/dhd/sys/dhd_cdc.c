...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
...
#elif ROAM_ENABLE_ITV
	uint roamvar = 0;
	int roam_trigger[2] = {-85, WLC_BAND_ALL};
	int roam_scan_period[2] = {10, WLC_BAND_ALL};
	int roam_delta[2] = {5, WLC_BAND_ALL};
	int roam_fullscan_period = 120;
#else
	uint roamvar = 1;
#endif
	...
	uint32 plcp = 0;
#endif
	uint32 spect_management = SPECT_MNGMT_OFF;
...
#if defined(ROAM_ENABLE) || defined(ROAM_ENABLE_ITV)
	...
	/* disable 11H spectrum management option per default */
	dhd_wl_ioctl_cmd(dhd, WLC_SET_SPECT_MANAGMENT, (char *)&spect_management, sizeof(spect_management), TRUE, 0);	
